...
namespace Parser.Blocks.Table
{
    ...
    public class TableRow
	{
		...
		internal static TableRow ParseHeader(string text)
		{
			...
			// TODO Parse TableRow
			var textCells = text.Split(new[] {"|"}, StringSplitOptions.RemoveEmptyEntries).Select(c => c.Trim())
				.ToList();

			foreach (var cell in textCells) cells.Add(TableCell.Parse(cell));
		...
		public override string ToString()
		{
			...
			foreach (var cell in Cells) textBuilder.Append($"| {cell} ");
