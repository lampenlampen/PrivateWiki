...
namespace Parser.Blocks
{
	public class TableBlock : Block
	{
		...
		internal static TableBlock Parse(List<string> lines, int blockStartLine = -1)
		{
			...
			for (var i = 2; i < lines.Count; i++) rows.Add(TableRow.Parse(lines[i]));
		...
		public override string ToString()
		{
			...
			// ColumnDefinitions
			textBuilder.Append("|");
			foreach (var definition in ColumnDefinitions) textBuilder.Append($"{definition}|");

			textBuilder.AppendLine("");

			// Table Data
			for (var i = 1; i < Rows.Count - 1; i++) textBuilder.AppendLine(Rows[i].ToString());
