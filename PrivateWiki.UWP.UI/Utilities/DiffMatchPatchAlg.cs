...
namespace PrivateWiki.UWP.UI.Utilities
{
	internal class DiffMatchPatchAlg { }
	...
	public class diff_match_patch
	{
			...
			DateTime deadline)
		{
			...
			{
				diffs = new List<Diff>();
				if (text1.Length != 0) diffs.Add(new Diff(Operation.EQUAL, text1));
			...
			if (commonprefix.Length != 0) diffs.Insert(0, new Diff(Operation.EQUAL, commonprefix));

			if (commonsuffix.Length != 0) diffs.Add(new Diff(Operation.EQUAL, commonsuffix));
			...
			bool checklines, DateTime deadline)
		{
			...
			if (checklines && text1.Length > 100 && text2.Length > 100) return diff_lineMode(text1, text2, deadline);
				...
				for (var k1 = -d + k1start; k1 <= d - k1end; k1 += 2)
				{
					...
					else if (front)
					{
						...
						if (k2_offset >= 0 && k2_offset < v_length && v2[k2_offset] != -1)
						{
							...
							if (x1 >= x2) return diff_bisectSplit(text1, text2, x1, y1, deadline);
						}
					}
				}
			...
			IList<string> lineArray)
		{
			...
			foreach (var diff in diffs)
			{
				...
				for (var j = 0; j < diff.text.Length; j++) text.Append(lineArray[diff.text[j]]);
		...
		public int diff_xIndex(List<Diff> diffs, int loc)
		{
			...
			foreach (var aDiff in diffs)
			{
				if (aDiff.operation != Operation.INSERT) chars1 += aDiff.text.Length;

				if (aDiff.operation != Operation.DELETE) chars2 += aDiff.text.Length;
			...
			if (lastDiff != null && lastDiff.operation == Operation.DELETE) return last_chars2;
		...
		public string diff_toDelta(List<Diff> diffs)
		{
			...
			if (delta.Length != 0) delta = delta.Substring(0, delta.Length - 1);
		...
		protected int match_bitap(string text, string pattern, int loc)
		{
			...
			for (var d = 0; d < pattern.Length; d++)
			{
				...
				if (match_bitapScore(d + 1, loc, loc, pattern) > score_threshold) break;
		...
		protected void patch_addContext(Patch patch, string text)
		{
			...
			var prefix = text.JavaSubstring(Math.Max(0, patch.start2 - padding),
				patch.start2);
			if (prefix.Length != 0) patch.diffs.Insert(0, new Diff(Operation.EQUAL, prefix));

			// Add the suffix.
			var suffix = text.JavaSubstring(patch.start2 + patch.length1,
				Math.Min(text.Length, patch.start2 + patch.length1 + padding));
			if (suffix.Length != 0) patch.diffs.Add(new Diff(Operation.EQUAL, suffix));
		...
		public List<Patch> patch_make(string text1, List<Diff> diffs)
		{
			...
			if (diffs.Count == 0) return patches; // Get rid of the null case.
			...
			foreach (var aDiff in diffs)
			{
				...
				if (aDiff.operation != Operation.INSERT) char_count1 += aDiff.text.Length;

				if (aDiff.operation != Operation.DELETE) char_count2 += aDiff.text.Length;
			}
		...
		{
			var text = new StringBuilder();
			foreach (var aPatch in patches) text.Append(aPatch);
